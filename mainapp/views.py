...
def recent_documents(request, scale = 'all'):
        last_five = Upload.objects.exclude(total__isnull=True).filter().order_by('-input_time')[:5].values('id','image','total','input_time','category','slug')
        # print(last_five.values('image').url)
        # images = last_five.values('image')
        # input_time = [i['input_time'].strftime("%m/%d/%y") for i in last_five.values('input_time')]
        # print(input_time, type(input_time))
        # total = last_five.values('total')
        # category = last_five.values('category')
        # slug = last_five.values('slug')
        # print(images)
        # docs = []
        # for n in range(len(last_five)):
            #list of dictionaries, might not be most efficient but is easier to iterate through in template and place all info.
            # docs.append({'image': Upload.objects.filter(id = id[n]['id']), 'input_time':input_time[n], 'total':total[n]['total'], 'category':category[n]['category']})

        return render(request, 'mainapp/document_scroller.html', {'docs':last_five, 'scale':scale})
